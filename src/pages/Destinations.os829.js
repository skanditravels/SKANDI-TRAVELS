...
import { getDestinationDetailPage } from "backend/destinationInventory.web";
...
function route(){const p=(Array.isArray(wixLocationFrontend.path)?wixLocationFrontend.path:[]).map(clean).filter(Boolean),i=p.lastIndexOf("destinations");return{countrySlug:p[i+1]||"",destinationSlug:p[i+2]||""}}
async function load(el,p={},id=""){const r={...route(),...p};const result=await getDestinationDetailPage({countrySlug:clean(r.countrySlug),destinationSlug:clean(r.destinationSlug),language:r.settings?.language||r.language||"EN",currency:r.settings?.currency||r.currency||"USD"});currentPage=result.page;send(el,"DESTINATION_PAGE_RESULT",{page:currentPage},id)}
...
$w.onReady(()=>{let el;try{el=$w(HTML_ID)}catch(e){console.error(e);return}el.onMessage(async ev=>{const m=parse(ev.data);if(!m||m.source!==HTML_SOURCE)return;const p=payload(m),id=String(m.requestId||p.requestId||"");try{if(m.type==="DESTINATION_READY"||m.type==="DESTINATION_REFRESH"){await load(el,p,id);return}if(m.type==="DESTINATION_SEARCH_OFFERS"){const r=await searchUnifiedOffers({search:liveSearch(p.search)});send(el,"DESTINATION_OFFERS_RESULT",{...(r||{}),items:Array.isArray(r?.items)?r.items:[]},id);return}if(m.type==="DESTINATION_SELECT_OFFER"){const r=await chooseOffer(obj(p.offer),liveSearch(p.search||p.searchContext||p.offer?.searchContext));send(el,"DESTINATION_OFFER_SELECTED",r,id);return}if(m.type==="DESTINATION_NAVIGATE")navigate(p.path)}catch(e){send(el,"DESTINATION_ERROR",{message:e?.message||"Destination request failed."},id)}});load(el).catch(e=>send(el,"DESTINATION_ERROR",{message:e.message}))});
