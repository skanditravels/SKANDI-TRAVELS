...
import { getPublicPassportVisaPayload, searchPublicTravelRequirements } from "backend/SKANDI_CORE/publicContent.web";
...
async function searchRequirements(e, p = {}) { try { send(e, "PASSPORT_VISA_SEARCHING", {}); send(e, "PASSPORT_VISA_SEARCH_RESULT", await searchPublicTravelRequirements({
...
})); } catch (x) { send(e, "PASSPORT_VISA_SEARCH_ERROR", { message: x?.publicMessage || x?.message || "Travel requirements search is unavailable." }); } }

$w.onReady(() => {
  const r = html(); if (!r) { console.error(`[Passport/Visa B-011.1] Missing embed: ${EMBED_IDS.join(", ")}`); return; }
  r.e.onMessage(async ev => { const m = parse(ev?.data); if (!m || (m.source && m.source !== SOURCE)) return; const p = m.payload || {}; if (["PASSPORT_VISA_READY", "PASSPORT_VISA_REFRESH"].includes(m.type)) await load(r.e, p); else if (m.type === "PASSPORT_VISA_SEARCH") await searchRequirements(r.e, p); else if (m.type === "PASSPORT_VISA_NAVIGATE") nav(p.path || m.path); });
