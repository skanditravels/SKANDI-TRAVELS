...
const HTML_ID="#holidayAreaHtml";
const HTML_SOURCE="SKANDI_DYNAMIC_DESTINATION_AREA";
...
async function load(el,p={},id=""){
  ...
  const destinationSlug=slug(p.destinationSlug||base.destinationSlug);
  const areaSlug=slug(p.areaSlug||p.slug||base.areaSlug);
  if(!countrySlug||!destinationSlug||!areaSlug)throw new Error("Missing country, destination or area in page URL.");
  const r=await getInventoryAreaPage({
    countrySlug,destinationSlug,areaSlug,
    language:p.settings?.language||p.language||"EN"
  });
  currentPage=r.page;
  send(el,"AREA_PAGE_RESULT",{page:currentPage,source:r.source},id);
}
...
$w.onReady(()=>{
  ...
  el.onMessage(async ev=>{
    ...
    try{
      ...
      if(m.type==="AREA_READY"||m.type==="AREA_REFRESH"){await load(el,p,id);return}
      if(m.type==="AREA_SEARCH_OFFERS"){
        const r=await searchUnifiedOffers({search:liveSearch(p.search)});
        send(el,"AREA_OFFERS_RESULT",{...(r||{}),items:Array.isArray(r?.items)?r.items:[]},id);return;
      }
      if(m.type==="AREA_SELECT_OFFER"){
      ...
      if(m.type==="AREA_NAVIGATE")navigate(p.path);
    }catch(e){send(el,"AREA_ERROR",{message:e?.publicMessage||e?.message||"Holiday area request failed."})}
  });
  sendCustomerHeaderState(el).catch(()=>{});
  setTimeout(()=>load(el).catch(e=>send(el,"AREA_ERROR",{message:e.message})),300);
});
