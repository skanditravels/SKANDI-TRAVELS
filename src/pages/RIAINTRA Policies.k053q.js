...
import { getInternalLegalDocument } from "backend/LEGAL/legalPolicyService.web";

const EMBED_ID = "#internalLegalPolicyEmbed";
...
async function load() {
  try {
    const result = await getInternalLegalDocument({ slug: queryValue("slug") });
    if (!result?.ok) {
      send("LEGAL_ERROR", { message: result?.message || "Internal policy unavailable." });
      return;
    }
    send("LEGAL_DOCUMENT_DATA", result);
  } catch (error) {
    send("LEGAL_ERROR", { message: error?.message || "Internal policy unavailable." });
  }
}
