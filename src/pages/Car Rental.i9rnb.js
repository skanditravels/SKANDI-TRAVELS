...
// SKANDI Car Rental — B-011.1 page bridge.
// Route: /car-rental
...
function cartAccess(payload = {}) {
  ...
  if (!cartId) {
    throw new Error("Open this rental through your owned SKANDI booking cart.");
  }
...
async function ensureCarCart(payload = {}) {
  ...
  if (currentCartId) {
    ...
    if (!payload.quoteId || cart.carQuote?.id === payload.quoteId) {
      return currentCartId;
    }
  }

  if (!payload.quoteId) {
    throw new Error("Select a current rental quote before booking.");
  }
...
function errorPayload(error) {
  return {
    code: String(error?.code || "CAR_RENTAL_ERROR"),
    message:
      error?.publicMessage ||
      error?.message ||
      "The car-rental request could not be completed."
  };
}

function setEmbedHeight(html, value) {
  const height = Number(value || 0);
  if (!Number.isFinite(height) || height <= 0) return;
  html.height = Math.max(900, Math.min(30000, Math.ceil(height)));
}

$w.onReady(() => {
  setCartId(
    wixLocation.query.cartId ||
    session.getItem("SKANDI_CAR_CART_ID") ||
    ""
  );
  ...
  html.onMessage(async event => {
    ...
    const mutating = [
      "CAR_RENTAL_BOOK",
      "CAR_RENTAL_CANCEL_BOOKING",
      "CAR_RENTAL_CREATE_CART",
      "CAR_RENTAL_SAVE_DRIVER",
      "CAR_RENTAL_PREPARE_CHECKOUT"
    ...
    try {
      if (message.type === "CAR_RENTAL_READY") {
        post(html, "CAR_RENTAL_BOOTSTRAP", {
          ...
          search: {
            pickupLocationText:
              wixLocation.query.pickup ||
              wixLocation.query.destination ||
              wixLocation.query.destinationSlug ||
              "",
            dropoffLocationText:
              wixLocation.query.dropoff ||
              wixLocation.query.pickup ||
              wixLocation.query.destination ||
              wixLocation.query.destinationSlug ||
              "",
            pickupDate:
              wixLocation.query.pickupDate ||
              wixLocation.query.departureDate ||
              "",
            dropoffDate:
              wixLocation.query.dropoffDate ||
              wixLocation.query.returnDate ||
              "",
      ...
      if (message.type === "CAR_RENTAL_HEIGHT") {
        setEmbedHeight(html, payload.height);
        return;
      }
      ...
      if (["CAR_RENTAL_GET_OFFER", "CAR_RENTAL_REPRICE"].includes(message.type)) {
        const result = await quoteDuffelCar({
          rateId: payload.rateId || payload.offerId || payload.id
        });

        post(
          html,
          message.type === "CAR_RENTAL_REPRICE"
      ...
      if (message.type === "CAR_RENTAL_PREPARE_SUPPLIER_PAYMENT") {
        ...
        post(html, "CAR_RENTAL_SUPPLIER_PAYMENT_READY", {
          ...
          instructions:
            "Complete any required card authorization through the secure supplier payment form."
        });
        return;
      }
      ...
      if (message.type === "CAR_RENTAL_CREATE_CART") {
        await ensureCarCart(payload);
        post(html, "CAR_RENTAL_CART_CREATED", {
          cartId: currentCartId
        });
        return;
      }
      ...
      if (message.type === "CAR_RENTAL_SAVE_DRIVER") {
        post(
          html,
          "CAR_RENTAL_DRIVER_SAVED",
          await saveCarDriver(cartAccess(payload))
        );
        return;
      }
      ...
      if (message.type === "CAR_RENTAL_PREPARE_CHECKOUT") {
        post(
          html,
          "CAR_RENTAL_CHECKOUT_READY",
          await prepareCarCheckout(cartAccess(payload))
        );
        return;
      }
      ...
      if (message.type === "CAR_RENTAL_BOOK") {
        const result = await createCustomerDuffelCarBooking({
          ...payload,
          alteaBookingId:
            payload.alteaBookingId ||
            wixLocation.query.bookingId ||
            ""
        });
      ...
      if (message.type === "CAR_RENTAL_LOOKUP_BOOKING") {
        const result = await getCustomerDuffelCarBooking(
          cartAccess({
            ...payload,
            bookingId:
              payload.bookingId ||
              payload.supplierBookingId
          })
        );
      ...
      if (message.type === "CAR_RENTAL_CANCEL_BOOKING") {
        const result = await cancelCustomerDuffelCarBooking(
          cartAccess({
            ...payload,
            bookingId:
              payload.bookingId ||
