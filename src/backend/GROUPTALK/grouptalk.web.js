...
async function realtimeServerConfig(){
  ...
  const keyType=key.startsWith("sb_secret_")?"modern-secret":key.startsWith("eyJ")?"legacy-jwt":"api-key";
...
function assertStaff(a){
  ...
  if(BLOCKED.has(lower(a.status)) || BLOCKED.has(lower(a.employment_status))) throw new Error("GROUPTALK_STAFF_BLOCKED");
...
function stationKey(agent){
  return upper(agent.station || agent.base || "NYC")
    .replace(/[^A-Z0-9]/g,"")
    .slice(0,20) || "NYC";
}

async function ensureDefaultGroup(agent){
  const existing=await allGroupRows();
  if(existing.length)return existing[0] || null;
  if(agent.can_manage!==true)return null;

  const station=stationKey(agent);
  const groupKey=`${station.toLowerCase()}-operations`;
  const stamp=now();

  const rows=await restRequest({
    table:T.groups,
    method:"POST",
    query:{on_conflict:"group_key"},
    body:{
      group_key:groupKey,
      name:`${station} Operations`,
      description:`SKANDI ${station} operational GroupTalk channel.`,
      channel_name:`grouptalk-${groupKey}`,
      pusher_channel:null,
      livekit_room:`grouptalk-${groupKey}`,
      ...
      payload:{
        ...
        allowAllStaff:true,
        defaultGroup:true,
        realtimeTopic:`grouptalk:${opaque()}`
      },
  ...
  if(saved){
    await audit("GROUPTALK_DEFAULT_GROUP_CREATED",{
      agent,
      group:saved,
      entityTable:T.groups,
      entityId:saved.id,
      after:saved
    });
    return saved;
  ...
  return first(await restRequest({
    table:T.groups,
    query:{select:"*",group_key:`eq.${groupKey}`,limit:1}
  }));
}
...
async function broadcast(group,event,payload={}){
  ...
  const {url,key,keyType}=await realtimeServerConfig(), topic=await ensureTopic(group);
  ...
  if(!res.ok){
    console.error("[GroupTalk] Supabase Realtime broadcast failed",{
      status:res.status,
      event,
      groupKey:group.group_key,
      response:String(raw||"").slice(0,240)
    });
    throw new Error(`SUPABASE_REALTIME_HTTP_${res.status}`);
  }
...
async function ensureCategories(agent){
  const current=await categoryRows(true);
  if(current.length)return current;

  const rows=DEFAULT_CATEGORIES.map(([category_key,label],i)=>({
    category_key,label,priority_default:"normal",sla_minutes:1440,
    is_active:true,sort_order:(i+1)*10,payload:{},
    created_by_agent_user_id:agent.id,created_at:now(),updated_at:now()
  }));

  await restRequest({
    table:T.categories,
    method:"POST",
    query:{on_conflict:"category_key"},
    body:rows,
...
async function startRealtimeSession(agent,group){
  ...
  return {sessionKey,topic,livekitIdentity};
}
...
export const getGroupTalkBootstrap = webMethod(Permissions.Anyone, async function(){
  const {agent,profile}=await requireStaff();

  await ensureDefaultGroup(agent);

  const [groups,cats,contacts]=await Promise.all([
    allowedGroups(agent),
    ensureCategories(agent),
    phonebook()
  ]);

  const active=groups.find(g=>g.defaultGroup)||groups[0]||null;
  const ticketQuery={select:"*",order:"updated_at.desc",limit:200};
  if(agent.can_manage!==true)ticketQuery.requester_agent_user_id=`eq.${agent.id}`;
  const ticketRows=await restRequest({table:T.tickets,query:ticketQuery});

  return {
    ok:true,
    provider:"SUPABASE",
    realtimeProvider:"SUPABASE_REALTIME",
    voiceProvider:"LIVEKIT",
    profile,
    groups,
    activeGroupId:active?.groupId||"",
    phonebook:contacts,
    tickets:(Array.isArray(ticketRows)?ticketRows:[]).map(publicTicket),
    ticketCategories:(Array.isArray(cats)?cats:[])
    ...
    config:{
      ...
      locationProvider:"WIX_WINDOW_FRONTEND",
      voiceRuntime:"WIX_CUSTOM_ELEMENT",
      presenceProvider:"SUPABASE_REALTIME_SESSIONS"
    },
    canManage:agent.can_manage===true
  };
});

export const getGroupTalkRealtimeConfig = webMethod(Permissions.Anyone, async function({groupId=""}={}){
  ...
  return {ok:true,provider:"SUPABASE_REALTIME",supabaseUrl:browser.url,publishableKey:browser.publishableKey,topic:session.topic,broadcastEvent:"grouptalk",sessionKey:session.sessionKey,livekitIdentity:session.livekitIdentity,groupId:g.groupId,groupKey:g.groupKey};
});

export const updateGroupTalkPresence = webMethod(Permissions.Anyone, async function({groupId="",sessionKey="",status="online",heartbeat=false}={}){
...
export const getGroupTalkPresence = webMethod(Permissions.Anyone, async function({groupId=""}={}){ const {agent}=await requireStaff(); const {row}=await requireGroup(agent,groupId); return {ok:true,members:await presence(row)}; });

export const createLiveKitToken = webMethod(Permissions.Anyone, async function({groupId="",sessionKey=""}={}){
  const {agent,profile}=await requireStaff();
  const {row:group,public:g}=await requireGroup(agent,groupId);
  ...
  const room=group.livekit_room||`grouptalk-${group.group_key}`;
  let identity=`${clean(profile.skId||agent.id,80)}-${randomPart(8)}`;

  const sessionLookup=clean(sessionKey,240);
  if(sessionLookup){
    const session=first(await restRequest({
      table:T.sessions,
      query:{
        select:"livekit_identity,agent_user_id,status",
        session_key:`eq.${sessionLookup}`,
        agent_user_id:`eq.${agent.id}`,
        limit:1
      }
    }));
    if(session?.livekit_identity)identity=session.livekit_identity;
  }
  ...
  token.addGrant({
    ...
    room,
    canSubscribe:true,
    canPublish:g.capabilities.canTalk===true,
    canPublishData:true
  });

  const jwt=await token.toJwt();
  await history("LIVEKIT_TOKEN_CREATED",{
    agent,group,livekitRoom:room,
    payload:{identity,canPublishAudio:g.capabilities.canTalk===true}
  });

  return {
    ok:true,token:jwt,livekitUrl,room,roomName:room,identity,groupId:group.id,
    canPublishAudio:g.capabilities.canTalk===true,canSubscribe:true
  };
});

export const triggerGroupTalkEvent = webMethod(Permissions.Anyone, async function(payload={}){
  const {agent,profile}=await requireStaff();
  const type=upper(payload.type||"EVENT");
  if(!ALLOWED_GROUPTALK_EVENTS.has(type))throw new Error("GROUPTALK_EVENT_NOT_ALLOWED");
...
export const getPhoneBook = webMethod(Permissions.Anyone, async function({groupId=""}={}){ const {agent}=await requireStaff(); if(groupId)await requireGroup(agent,groupId); return {ok:true,contacts:await phonebook()}; });

export const sendLocationPing = webMethod(Permissions.Anyone, async function(payload={}){
...
export const getLiveLocations = webMethod(Permissions.Anyone, async function({groupId=""}={}){
...
export const createGroupTalkTicket = webMethod(Permissions.Anyone, async function(payload={}){
...
export const getGroupTalkTickets = webMethod(Permissions.Anyone, async function(payload={}){
...
export const replyToGroupTalkTicket = webMethod(Permissions.Anyone, async function(payload={}){
...
export const searchGroupTalkHistory = webMethod(Permissions.Anyone, async function({query="",groupId="",limit=200}={}){
...
export const adminSaveGroup = webMethod(Permissions.Anyone, async function(payload={}){
...
export const adminSetMembership = webMethod(Permissions.Anyone, async function(payload={}){
...
export const getTicketCategories = webMethod(Permissions.Anyone, async function(){ const {agent}=await requireStaff(); const rows=await ensureCategories(agent); return {ok:true,categories:(Array.isArray(rows)?rows:[]).filter(r=>r.is_active!==false).sort((a,b)=>Number(a.sort_order||100)-Number(b.sort_order||100)).map(publicCategory)}; });

export const saveTicketCategory = webMethod(Permissions.Anyone, async function(payload={}){
...
export const deleteTicketCategory = webMethod(Permissions.Anyone, async function(payload={}){
...
export const getGroupTalkDiagnostics = webMethod(Permissions.Anyone, async function(){
  const result={
    ok:false,checkedAt:now(),staff:false,supabaseRest:false,groups:0,categories:0,
    realtimeServerKey:false,realtimeBrowserKey:false,
    livekitUrl:false,livekitApiKey:false,livekitApiSecret:false
  };
  ...
  result.groups=(await allGroupRows()).length;
  result.categories=(await categoryRows(true)).length;
  result.supabaseRest=true;

  const [rtUrl,rtSecret,rtLegacy,rtPublishable,rtAnon,lkUrl,lkKey,lkSecret]=await Promise.all([
    secret("SUPABASE_URL"),secret("SUPABASE_SECRET_KEY"),secret("SUPABASE_SERVICE_ROLE_KEY"),
    secret("SUPABASE_PUBLISHABLE_KEY"),secret("SUPABASE_ANON_KEY"),
    secret("LIVEKIT_URL"),secret("LIVEKIT_API_KEY"),secret("LIVEKIT_API_SECRET")
  ]);

  result.realtimeServerKey=Boolean(rtUrl&&(rtSecret||rtLegacy));
  result.realtimeBrowserKey=Boolean(rtUrl&&(rtPublishable||rtAnon));
  result.livekitUrl=Boolean(lkUrl);
  result.livekitApiKey=Boolean(lkKey);
  result.livekitApiSecret=Boolean(lkSecret);
  result.ok=result.staff&&result.supabaseRest&&result.groups>0&&
    result.realtimeServerKey&&result.realtimeBrowserKey&&
    result.livekitUrl&&result.livekitApiKey&&result.livekitApiSecret;
