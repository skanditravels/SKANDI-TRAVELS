...
function normalizePolicyStatus(value, fallback = "Draft") {
  ...
  if (POLICY_STATUSES.has(raw)) {
    return raw;
  }

  const match = [...POLICY_STATUSES].find(
...
function sanitizeSections(input, depth = 0) {
  ...
  return safeArray(input)
    ...
    .map((item) => ({
      ...
      bodyHtml: sanitizeRichHtml(
        item?.bodyHtml ||
        item?.body_html ||
        ""
      ),
      children: sanitizeSections(
        item?.children,
        depth + 1
      )
    }));
...
function buildDocumentHtml(
  ...
  return {
    bodyHtml:
      `${introductionHtml || ""}${sectionsHtml}`,

    bodyPlainText:
      `${htmlToPlain(introductionHtml)}

${flat
  .map(
    (item) =>
      `${item.label} ${item.title}
${htmlToPlain(item.bodyHtml)}`
  )
  .join("\n\n")}`.trim(),
...
function addMonths(dateValue, months) {
  ...
  const date = new Date(
    `${String(dateValue).slice(0, 10)}T12:00:00Z`
  );
...
function reviewStatus(reviewDate) {
  ...
  const date = new Date(
    `${String(reviewDate).slice(0, 10)}T00:00:00`
  );
...
async function requirePolicyAdmin() {
  ...
  const allowed = Boolean(
    ...
    profile.permissions?.manage === true ||
    profile.access?.manage === true ||
    session.permissions?.manage === true ||
    tokens.some(
      (token) =>
        ADMIN_TOKENS.has(
          token.trim()
  ...
  if (!allowed) {
    throw new Error(
      "Policy Control permission is required."
    );
  }
...
function routeFor(policy = {}) {
  const scope =
    normalizeScope(policy.scope);

  if (scope === "internal") {
    return (
      "/riaintra/success-factors/" +
      "legal-internal/policies-internal" +
...
function toDatabase(
  ...
  const introductionHtml =
    sanitizeRichHtml(
      source.introductionHtml ||
      source.introduction_html ||
      ""
    );
  ...
  const reviewIntervalMonths =
    reviewMode === "months"
      ? Math.max(
          1,
          numberValue(
            source.reviewIntervalMonths ||
            source.review_interval_months,
            12
          )
        )
      : null;
  ...
  if (!sections.length) {
    throw new Error(
      "Add at least one numbered section."
    );
  }
  ...
  return {
    ...
    pdf_file_url:
      contentChanged
        ? null
        : existing?.pdf_file_url || null,

    pdf_file_name:
      contentChanged
        ? null
        : existing?.pdf_file_name || null,

    pdf_generated_at:
      contentChanged
        ? null
        : existing?.pdf_generated_at || null,
    ...
    source_pdf_name:
      cleanText(
        source.sourcePdfName ||
        source.source_pdf_name ||
        existing?.source_pdf_name,
        240
      ) || null,
...
async function saveRevision({
  ...
  await sbInsert(
    REVISION_TABLE,
    {
      ...
      previous_snapshot:
        previous
          ? snapshotForHistory(previous)
          : null,

      snapshot:
        snapshotForHistory(next),
...
async function selectAllPolicies() {
  return (
    await sbSelect(
      POLICY_TABLE,
      "select=*&order=sort_order.asc,updated_at.desc"
    )
  ) || [];
}
...
async function ensureUnique(
  ...
  const rows =
    await selectAllPolicies();

  const conflict =
    rows.find((row) => {
      if (
        row.policy_id === currentId
      ) {
        return false;
      }
  ...
  if (conflict) {
    throw new Error(
      "Document ID, slug, and external policy slot must be unique."
    );
  }
}
...
function parsePolicyText(
  ...
  const lines =
    rawLines.filter((line) => {
      ...
      if (
        /^Issued to .+\| Version .+\|.+\| Page \d+ of \d+$/i
          .test(line)
      ) {
        return false;
      }
  ...
  const effectiveMatch =
    joined.match(
      /Effective Date:\s*([A-Za-z]{3,9}\s+\d{1,2},\s+\d{4}|\d{4}-\d{2}-\d{2})/i
    );

  const versionMatches = [
    ...joined.matchAll(
      /Version:\s*([0-9]+(?:\.[0-9]+)*)/gi
    )
  ];

  const updatedByMatch =
    joined.match(
      /(?:Updated By|Approved By):\s*([A-Z]{2}\d{4,}|[A-Za-z0-9-]+)/i
    );
  ...
  const brand =
    /\bALTEA\b/i.test(joined) &&
    !/SKANDI TRAVELS/i.test(joined)
      ? "ALTEA"
      : "SKANDI TRAVELS";
  ...
  const title =
    lines.find((line) => {
      ...
      if (
        /^(SKANDI|ALTEA|Effective Date:|Version:|Last Updated:|Updated By:|Owner:|Notes:)/i
          .test(line)
      ) {
        return false;
      }
      ...
      return (
        /(Policy|Procedure|Terms|Statement|Notice|Standard|Guide)$/i
          .test(line)
      );
    }) ||
    cleanText(
  ...
  lines.forEach((line) => {
    ...
    if (
      /^(Document Control|Version:|Last Updated:|Updated By:|Approved By:|Owner:|Notes:)/i
        .test(line)
    ) {
      return;
    }
...
export const getPublicLegalHub =
  webMethod(
    Permissions.Anyone,
    async () => {
      const rows =
    ...
    async (input = {}) => {
      ...
      if (!row) {
        return {
          ok: false,
          message:
            "This legal document is not published or is unavailable."
        };
      }
...
export const getInternalLegalHub =
  webMethod(
    ...
    async () => {
      ...
      const rows =
        await sbSelect(
          POLICY_TABLE,
          "select=*&scope=eq.internal&status=eq.Published&active=eq.true&order=sort_order.asc,title.asc"
        );
...
export const getInternalLegalDocument =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const {
      ...
      const rows =
        await sbSelect(
          POLICY_TABLE,
          `select=*&scope=eq.internal&status=eq.Published&active=eq.true&${eq(
            "slug",
            slug
          )}&limit=1`
        );
      ...
      if (!row) {
        return {
          ok: false,
          message:
            "This internal policy is not published or is unavailable."
        };
      }
...
export const getPolicyAdminBootstrap =
  webMethod(
    Permissions.SiteMember,
    async () => {
      const profile =
...
export const adminListLegalPolicies =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      await requirePolicyAdmin();
...
export const adminGetLegalPolicy =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      await requirePolicyAdmin();
...
export const adminSaveLegalPolicy =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const profile =
      ...
      if (existing) {
        ...
        await saveRevision({
          ...
          summary:
            ...
            (
              contentChanged
                ? "Policy content changed and version advanced automatically."
                : "Policy saved without a version change."
            )
        });
      } else {
        const rows =
...
export const adminPublishLegalPolicy =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const profile =
...
export const adminArchiveLegalPolicy =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const profile =
...
export const adminDeleteLegalPolicy =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const profile =
      ...
      await saveRevision({
        ...
        summary:
          input.changeSummary ||
          "Policy removed from use. The record and full audit history were retained."
      });
      ...
      return {
        ...
        message:
          "Policy deleted from use. Audit history retained."
      };
    }
  );

export const adminRegenerateLegalPolicyPdf =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      const profile =
      ...
      await saveRevision({
        ...
        summary:
          input.changeSummary ||
          "Standard letter-size PDF regenerated."
      });
      ...
      return {
        ...
        message:
          result.warning
            ? "Policy PDF generation failed."
            : "Policy PDF regenerated.",
...
export const adminParseLegalPolicyPdfText =
  webMethod(
    Permissions.SiteMember,
    async (input = {}) => {
      await requirePolicyAdmin();
      ...
      return {
        ...
        message:
          "PDF text analyzed. Review all imported fields before saving."
      };
    }
  );
