...
export const searchDuffelOrderChanges = secure(async (input={}) => {
...
export const createDuffelPendingOrderChange = secure(async (input={}) => {
...
export const getDuffelPendingOrderChange = secure(async (input={}) => {
...
export const prepareDuffelOrderChangePayment = secure(async (input={}) => {
  const changeId = genericResourceId(input.orderChangeId,"order change");
...
export const confirmDuffelOrderChange = secure(async (input={}) => {
  const changeId = genericResourceId(input.orderChangeId,"order change");
  const raw = (await duffelRequest(`/air/order_changes/${encodeURIComponent(changeId)}`)).data;
  assertPendingChange(raw);
...
function secure(handler) {
  return webMethod(Permissions.SiteMember, async (input={}) => {
    try {
      ...
      if (!session?.loggedIn || !session?.authorized) throw portalError("AUTH_REQUIRED","An authorized SKANDI staff session is required.");
