// /src/backend/SKANDI_CORE/duffelGroundProducts.js
// SKANDI R-006.2 — reusable Duffel Stays/Cars provider core.
// Customer/member and staff authorization checks remain inside the relevant core functions.

import { currentMember } from "wix-members-backend";
import { duffelRequest, ProviderError } from "./duffelClient.js";
...
function eq(field, value) {
  return `${encodeURIComponent(String(field || ""))}=eq.${encodeURIComponent(String(value ?? ""))}`;
}

function parseLegacyQuery(queryString = "") {
  const query = {};
  for (const part of String(queryString || "").split("&")) {
    if (!part) continue;
...
async function sbSelect(table, queryString = "") {
  const result = await restRequest({
    table,
    method: "GET",
    query: parseLegacyQuery(queryString),
    prefer: ""
  });
...
export const searchDuffelStaysCore = async input => searchStaysInternal(input || {});


export const fetchDuffelStayRatesCore = async ({ searchResultId = "" } = {}) => {
...
export const quoteDuffelStayCore = async ({ rateId = "" } = {}) => {
...
export const searchDuffelCarsCore = async (input = {}) => {
...
export const quoteDuffelCarCore = async ({ rateId = "" } = {}) => {
...
export const createDuffelComponentClientKeyCore = async () => {
  await requireMember();
...
export const createCustomerDuffelCarBookingCore = async (input = {}) => {
...
export const getCustomerDuffelCarBookingCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  const member = await requireMember();
...
export const cancelCustomerDuffelCarBookingCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  const member = await requireMember();
  if (!isUuid(alteaBookingId)) throw publicError("BOOKING_REQUIRED", "Choose the SKANDI trip containing this car rental.");
  const owned = await sbSelect(BOOKINGS, `select=id,customer_member_id&${eq("id", alteaBookingId)}&limit=1`);
  if (!owned?.[0] || clean(owned[0].customer_member_id, 100) !== member._id) throw publicError("BOOKING_ACCESS_DENIED", "That booking does not belong to this account.");
...
export const createCustomerDuffelStayBookingCore = async (input = {}) => {
  const member = await requireMember();
...
export const getCustomerDuffelStayBookingCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  const member = await requireMember();
...
export const cancelCustomerDuffelStayBookingCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  const member = await requireMember();
  if (!isUuid(alteaBookingId)) throw publicError("BOOKING_REQUIRED", "Choose the SKANDI trip containing this hotel.");
  const owned = await sbSelect(BOOKINGS, `select=id,customer_member_id&${eq("id", alteaBookingId)}&limit=1`);
  if (!owned?.[0] || clean(owned[0].customer_member_id, 100) !== member._id) throw publicError("BOOKING_ACCESS_DENIED", "That booking does not belong to this account.");
...
export const createDuffelComponentClientKeyStaffCore = async () => {
  await requireStaff();
  const response = await duffelRequest("/identity/component_client_keys", { method: "POST", body: {} });
  return { componentClientKey: clean(response?.data?.component_client_key, 5000) };
};
...
export const createDuffelStayBookingStaffCore = async (input = {}) => {
  await requireStaff();
...
export const getDuffelStayBookingStaffCore = async ({ bookingId = "" } = {}) => {
  await requireStaff();
...
export const cancelDuffelStayBookingStaffCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  await requireStaff();
  const id = resourceId(bookingId, "bok_", "stay booking");
...
export const getDuffelCarBookingStaffCore = async ({ bookingId = "" } = {}) => {
  await requireStaff();
...
export const createDuffelCarBookingStaffCore = async (input = {}) => {
  await requireStaff();
...
export const cancelDuffelCarBookingStaffCore = async ({ bookingId = "", alteaBookingId = "" } = {}) => {
  await requireStaff();
  const id = resourceId(bookingId, "boo_", "car booking");
...
export const syncDuffelStayBookingToAlteaCore = async ({ alteaBookingId = "", stayBooking = null, bookingId = "" } = {}) => {
  await requireStaff();
  let b = stayBooking;
...
export const syncDuffelCarBookingToAlteaCore = async ({ alteaBookingId = "", carBooking = null, bookingId = "" } = {}) => {
  await requireStaff();
  let b = carBooking;
