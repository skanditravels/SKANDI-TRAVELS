...
function normalizeMaster(row={}){
  return{
    ...
    searchKeywords:array(row.search_keywords),sortPriority:nullableNumber(row.sort_priority,{integer:true})??100,
...
function normalizeAirline(row={},includeSourceRecord=true){
  return{
    ...
    sourceTable:"travel_info_airlines",sourceId:clean(row.ID,100),
    ...(includeSourceRecord?{sourceRecord:row}:{}),
...
async function supportedLanguages(){
  const rows=await selectAllPaged("inventory_localized_content",{select:"language",order:"language.asc,id.asc",limit:"5000"});
...
export async function getInventoryBootstrapCore(input={}){
  ...
  const [canonical,sources,catalog,airlinesRaw,aircraftRaw,dated,flight,classes,schedule,nesting,languages]=await Promise.all([
    selectAllPaged("inventory_canonical_entities_v",{
      ...
      order:"entity_type.asc,sort_priority.asc,id.asc",limit:"3000"
    }),
    selectAllPaged("inventory_source_registry",{select:"*",active:"eq.true",order:"category.asc,canonical_table.asc",limit:"250"}),
    catalogRows(),
    selectAllPaged("travel_info_airlines",{
      select:airlineBootstrapSelect,
      active:"eq.true",order:"sort_order.asc,ID.asc",limit:"250"
    }),
    selectAllPaged("travel_info_aircraft",{
      select:aircraftBootstrapSelect,
      order:"airline_code.asc,sort_order.asc,id.asc",limit:"1200"
    }),
  ...
  return{
    ...
    // Fleet template JSON stays server-side for quality checks. Record editors
    // still receive the complete source row through INVENTORY_V9_GET_RECORD.
    airlines:airlinesRaw.map(row=>normalizeAirline(row,false)),aircraft:aircraftRaw.map(normalizeAircraft),
...
function mediaBody(entityId,row={},index=0){
  ...
  const rawSourceKind=upper(row.sourceKind??row.source_kind??"MIGRATED",40);
  const sourceKind=rawSourceKind==="ASSET_LIBRARY"?"UPLOAD":(["URL","UPLOAD","MIGRATED"].includes(rawSourceKind)?rawSourceKind:"MIGRATED");
  return{
...
async function resolveAssetBackedMedia(entityId,row={},index=0){
  const payload=object(row.payload);
  const assetId=clean(row.assetId??row.platformAssetId??payload.assetId,80);
  if(assetId){
    if(!isUuid(assetId))throw new Error("INVENTORY_ASSET_ID_INVALID");
    const asset=(await select("platform_assets",{select:"*",id:qeq(assetId),status:"eq.ACTIVE",limit:"1"}))[0];
    if(!asset)throw new Error("INVENTORY_ASSET_NOT_FOUND");
    if(upper(asset.visibility,20)!=="PUBLIC")throw new Error("INVENTORY_PUBLIC_ASSET_REQUIRED");
    const url=clean(asset.public_url,3000);
    if(!url)throw new Error("INVENTORY_ASSET_PUBLIC_URL_MISSING");
    const body=mediaBody(entityId,{...row,
      url,storageBucket:asset.bucket,storagePath:asset.storage_path,mimeType:asset.mime_type,
      fileSizeBytes:asset.size_bytes,sourceKind:"UPLOAD",
      payload:{...payload,assetId:asset.id,assetCode:asset.asset_code}
    },index);
    return body;
  }
  // Existing legacy/migrated rows remain saveable until explicitly replaced.
  // New Inventory media must originate from the central Asset Library.
  if(!clean(row.id,80))throw new Error("INVENTORY_ASSET_LIBRARY_REQUIRED");
  return mediaBody(entityId,{...row,sourceKind:upper(row.sourceKind??row.source_kind??"MIGRATED",40)},index);
}

async function resolveReferenceMediaRows(type,id,rows=[]){
  const sourceTable=type==="AIRLINE"?"travel_info_airlines":"travel_info_airports";
  const current=(await select(sourceTable,{select:"media_assets",ID:qeq(id),limit:"1"}))[0];
  const existingUrls=new Set(array(current?.media_assets).map(x=>clean(x?.url,3000)).filter(Boolean));
  const resolved=[];
  for(let i=0;i<array(rows).length;i++){
    const row=array(rows)[i];
    const payload=object(row?.payload);
    const assetId=clean(row?.assetId??row?.platformAssetId??payload.assetId,80);
    if(assetId){
      if(!isUuid(assetId))throw new Error("INVENTORY_ASSET_ID_INVALID");
      ...
      if(upper(asset.visibility,20)!=="PUBLIC")throw new Error("INVENTORY_PUBLIC_ASSET_REQUIRED");
      resolved.push({
        ...row,url:clean(asset.public_url,3000),storageBucket:clean(asset.bucket,180),
        storagePath:clean(asset.storage_path,1000),mimeType:clean(asset.mime_type,160),
        fileSizeBytes:nullableNumber(asset.size_bytes,{integer:true,min:0}),sourceKind:"UPLOAD",
        payload:{...payload,assetId:asset.id,assetCode:asset.asset_code}
      });
      continue;
    }
    const url=clean(row?.url,3000);
    if(url&&existingUrls.has(url)){resolved.push({...row,sourceKind:"MIGRATED"});continue;}
    throw new Error("INVENTORY_ASSET_LIBRARY_REQUIRED");
  }
  return resolved;
}

async function resolvePublicAssetUrl(assetRef,currentUrl="",desiredUrl="",{required=false}={}){
  const ref=object(assetRef);
  const assetId=clean(ref.id??ref.assetId,80);
  const desired=clean(desiredUrl,3000);
  const current=clean(currentUrl,3000);
  if(assetId){
    if(!isUuid(assetId))throw new Error("INVENTORY_ASSET_ID_INVALID");
    const asset=(await select("platform_assets",{select:"id,asset_code,visibility,bucket,storage_path,mime_type,size_bytes,public_url,status",id:qeq(assetId),status:"eq.ACTIVE",limit:"1"}))[0];
    if(!asset)throw new Error("INVENTORY_ASSET_NOT_FOUND");
    if(upper(asset.visibility,20)!=="PUBLIC")throw new Error("INVENTORY_PUBLIC_ASSET_REQUIRED");
    const url=clean(asset.public_url,3000);
    if(!url)throw new Error("INVENTORY_ASSET_PUBLIC_URL_MISSING");
  ...
  if(!desired){
    if(required)throw new Error("INVENTORY_ASSET_LIBRARY_REQUIRED");
    return{url:"",asset:null};
  }
  // Existing stored URLs may be preserved during an unrelated edit. Any new/replaced
  // image URL must come from platform_assets through the Asset Library picker.
  if(current && desired===current)return{url:desired,asset:null};
  throw new Error("INVENTORY_ASSET_LIBRARY_REQUIRED");
}

async function resolveAircraftAssetFields(aircraft={},current={}){
  const refs=object(aircraft._assetRefs);
  const map=[
    ["heroImageUrl","hero_image_url"],
    ["exteriorImageUrl","exterior_image_url"],
    ["seatmapImageUrl","seatmap_image_url"],
    ["thumbnailImageUrl","thumbnail_image_url"]
  ];
  const out={};
  for(const [camel,snake] of map){
    const desired=aircraft[camel]??aircraft[snake]??"";
    const result=await resolvePublicAssetUrl(refs[camel]??refs[snake],current?.[snake],desired,{required:false});
    out[snake]=result.url||null;
  }
  return out;
}

async function resolveAircraftChildAssetFields(kind,item={},current={}){
  const refs=object(item._assetRefs);
  const fields=kind==="view"
    ? [["image_url",true],["mobile_image_url",false],["thumbnail_url",false]]
    : kind==="scene"
      ? [["image_url",true],["mobile_image_url",false]]
      : kind==="hotspot"
        ? [["thumbnail_url",false]]
        : [];
  const out={};
  for(const [field,required] of fields){
    ...
    const desired=item[field]??item[camel]??"";
    const result=await resolvePublicAssetUrl(refs[field]??refs[camel],current?.[field],desired,{required});
    out[field]=result.url||null;
  }
  return out;
}
...
async function syncMasterChildren(session,record,bundle){
  ...
  if(Array.isArray(bundle.media)){
    ...
    for(let i=0;i<bundle.media.length;i++){
      const item=bundle.media[i],body=await resolveAssetBackedMedia(entityId,item,i);
  ...
  // Catalog entry is the merchandising child record; canonical search/collection columns on
  // inventory_master_entities mirror it so all consumers read a synchronized projection.
  const featured=catalog?.featured===true||catalog?.homepageFeatured===true;
  const home=catalog?.homepageFeatured===true;
  await restRequest({table:"inventory_master_entities",method:"PATCH",query:{id:qeq(entityId)},body:{
    featured,homepage_featured:home,searchable:catalog?.searchable===true,
    collection_type:catalog?.catalogType&&catalog.catalogType!=="NONE"?catalog.catalogType:"NONE",
    partner_tier:catalog?.partnerTier||null,search_priority:catalog?.searchPriority??100,
    search_keywords:array(catalog?.searchKeywords),updated_by_agent_user_id:actorId(session)
...
async function syncReferenceChildren(session,record,bundle){
  ...
  const catalog=await saveCatalog(session,record,bundle.catalog);
  const resolvedMedia=Array.isArray(bundle.media)?await resolveReferenceMediaRows(type,id,bundle.media):array(record.media);
  const patch={
    localized_content:Array.isArray(bundle.localizedContent)?bundle.localizedContent:array(record.localizedContent),
    media_assets:resolvedMedia,
...
export async function getAircraftBootstrapCore(){
  ...
  return{ok:true,version:INVENTORY_CORE_VERSION,airlines:airlines.map(row=>normalizeAirline(row)),aircraft:aircraft.map(normalizeAircraft),sync:syncHealth(airlines,aircraft)};
}
...
export async function saveAircraftCore(input={}){
  ...
  const current=id?(await select("travel_info_aircraft",{select:"*",id:qeq(id),limit:"1"}))[0]||{}:{};
  const resolvedAssets=await resolveAircraftAssetFields(a,current);
  ...
  const body={
    ...
    hero_image_url:resolvedAssets.hero_image_url,
    exterior_image_url:resolvedAssets.exterior_image_url,
    seatmap_image_url:resolvedAssets.seatmap_image_url,
    thumbnail_image_url:resolvedAssets.thumbnail_image_url,
...
export async function saveAircraftChildCore(input={}){
  ...
  const id=clean(item.id,80);
  const current=id?(await select(spec.table,{select:"*",id:qeq(id),limit:"1"}))[0]||{}:{};
  const assetFields=await resolveAircraftChildAssetFields(kind,item,current);
  const body=aircraftChildBody(kind,{...item,...assetFields});
  let saved;
  if(id)saved=(await restRequest({table:spec.table,method:"PATCH",query:{id:qeq(id)},body,prefer:"return=representation"}))?.[0];
